...
var Carousel = function (_MLP$apps$MLPModule) {
  ...
  _createClass(Carousel, [{
    ...
    /*animate(inx, callback) {
      var deviceWidth = $('body').width();
      if(inx > (this.el.slice.length)){
        this.el.topCarousel.css('left', 0);
        inx = 1;
      }
      if(inx <0){
        this.el.topCarousel.css('left', (-(deviceWidth * this.el.slice.length) + 'px'));
        inx = this.el.slice.length - 1;
      }
      this.el.topCarousel.animate({
        left: -(deviceWidth * inx) + 'px'
      }, () => {
